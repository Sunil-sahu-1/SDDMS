...
export async function apiRequest<T>(
 ...
 if (token && !skipAuth) {
  ...
  const response = await fetch(`${API_BASE_URL}${path.startsWith("/") ? path : `/${path}`}`, { ...options, headers, cache: "no-store" });
  const text = await response.text();
  let data: unknown = null;
  try { data = text ? JSON.parse(text) : null; } catch { data = text; }
  if (!response.ok) {
    if (response.status === 401 && typeof window !== "undefined") localStorage.removeItem("sddms_access_token");
    const body = data as {detail?: string; message?: string} | null;
    throw new ApiError(body?.detail ?? body?.message ?? `Request failed (${response.status})`, response.status, data);
  }
  return data as T;
}
...
export const authApi = {
  ...
  register: (payload: Record<string, unknown>) => apiRequest('/accounts/register/', {method:'POST', body: JSON.stringify(payload)}),
 registerApplication: (data: FormData) =>
  ...
  verify: (payload: Record<string, unknown>) => apiRequest('/accounts/verification/', {method:'POST', body: JSON.stringify(payload)}),
};
...
export const evidenceApi = {
  ...
  download: (id: string|number) => apiRequest(`/evidence/${id}/download/`),
...
export const auditApi = { list: (query="") => apiRequest(`/audit/logs/${query ? `?${query}` : ""}`) };

export const dashboardApi = { get: () => apiRequest("/admin/dashboard/") };
...
export const searchApi = { global: (q: string) => apiRequest(`/search/?q=${encodeURIComponent(q)}`) };
export const aiApi = {
