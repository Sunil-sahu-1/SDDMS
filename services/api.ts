...
export async function apiDownload(path: string): Promise<Response> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      data?.detail ?? data?.message ?? `Download failed (${response.status})`,
      response.status,
      data,
    );
  }
...
export const documentsApi = {
  ...
  newVersion: (id: string | number, data: FormData) =>
    apiRequest(`/documents/${id}/new-version/`, { method: "POST", body: data }),
};
...
export const legalApi = {
  ...
  rejectReview: (id: string | number, reason?: string) =>
    apiRequest(`/legal/reviews/${id}/reject/`, { method: "POST", body: JSON.stringify({ rejection_reason: reason }) }),
};
